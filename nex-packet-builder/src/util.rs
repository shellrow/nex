...
use nex_packet::ethernet::{EtherType, MutableEthernetPacket};
use nex_packet::icmp::ICMPV4_HEADER_LEN;
use nex_packet::icmpv6::ICMPV6_HEADER_LEN;
use nex_packet::ip::IpNextLevelProtocol;
use nex_packet::ipv4::{MutableIpv4Packet, IPV4_HEADER_LEN};
use nex_packet::ipv6::{MutableIpv6Packet, IPV6_HEADER_LEN};
...
pub fn build_full_icmp_packet(packet_option: PacketBuildOption) -> Vec<u8> {
    ...
    let mut icmp_packet =
        nex_packet::icmp::echo_request::MutableEchoRequestPacket::new(&mut icmp_buffer).unwrap();
...
pub fn build_full_icmpv6_packet(packet_option: PacketBuildOption) -> Vec<u8> {
    ...
    let mut icmpv6_packet =
        nex_packet::icmpv6::echo_request::MutableEchoRequestPacket::new(&mut icmpv6_buffer)
            .unwrap();
